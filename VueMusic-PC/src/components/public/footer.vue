<template>
    <div class="music-footer" ref="footer" :id="theme">
        <audio autoplay controls :src="url" v-show="false" ref="audio" @ended="next(false)" @timeupdate="timeupdate">
        </audio>
        ...
        <v-slider :value="curTimeNum" :totalVal="tolTimeNum" :width="width" @skip="skip" @move="move"></v-slider>
        ...
        <div class="list-dialog" v-if="showDialog">
            ...
            <ul v-if="musicList && musicList.length !== 0">
                <li v-for="(item, index) in musicList" @dblclick="changeMusic(index)">
                    ...
                    <span class="singer" @click="$router.push({name: 'singer', params:{id: item.singerId}});
              $store.commit('setShowPlay', false)">{{item.singer}}</span>
